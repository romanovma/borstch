...
var WolfImage = React.createClass({displayName: "WolfImage",
  ...
  handleClick: function(e) {
      var imgPos= $(e.currentTarget).position();
      var horPos = e.pageX - imgPos.left - e.currentTarget.width/2;
      var verPos = e.pageY - imgPos.top - e.currentTarget.height/2;
      if (horPos > 0 && verPos < 0) {this.setState({position: UP_RIGHT})}
      else if (horPos > 0 && verPos > 0) {this.setState({position: DOWN_RIGHT})}
      else if (horPos<0 && verPos > 0) {this.setState({position: DOWN_LEFT})}
      else {this.setState({position: UP_LEFT})};
  },
  render: function() {
    var imgStyle = {
      width: '100%'
