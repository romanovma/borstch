...
var GameBox = React.createClass({displayName: "GameBox",
  getInitialState: function() {
    return {
      ...
      total: 1,
      runningUP_LEFT: true,
      runningDOWN_LEFT: false,
      runningUP_RIGHT: false,
      runningDOWN_RIGHT: false
    };
  },
  ...
  pushFood: function() {
    ...
    switch (rnd % 4) {
      case 1:
        this.setState({runningUP_LEFT: true});
        break;
      case 2:
        this.setState({runningDOWN_LEFT: true});
        break;
      case 3:
        this.setState({runningUP_RIGHT: true});
        break;
      case 4:
        this.setState({runningDOWN_RIGHT: true});
        break;
      default:
    }
  },
  ...
  incrTotal: function (element) {
    ...
    switch (element) {
      case UP_LEFT:
        this.setState({runningUP_LEFT: false});
        break;
      case UP_RIGHT:
        this.setState({runningDOWN_LEFT: false});
        break;
      case DOWN_LEFT:
        this.setState({runningUP_RIGHT: false});
        break;
      case DOWN_RIGHT:
        this.setState({runningDOWN_RIGHT: false});
        break;
      default:
...
var Shelve = React.createClass({displayName: "Shelve",
  ...
  drawText: function(canvas, ctx) {
    ...
    if (this.props.position === UP_LEFT || this.props.position === DOWN_LEFT) {
      ...
      ctx.fillText("свекла", canvas.width / 5 * (5 - this.state.level) - 20, canvas.height / 5 * this.state.level - 20);
    }
  },
  componentDidMount: function () {
