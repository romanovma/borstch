...
var GameBox = React.createClass({displayName: "GameBox",
  ...
  incrTotal: function (position, food) {
    console.log(food in this.props.allowable);
    console.log(position);
    if (position === this.state.pose && food in this.props.allowable)
    {
      var newTotal = this.state.total + 1;
      this.setState({total: newTotal});
    } else if (position === this.state.pose && food in this.props.notAllowable){
    ...
    return (
      React.createElement("div", null, 
        React.createElement("div", {className: "row"}, 
          React.createElement("h1", null, " ", this.state.header), 
          this.state.total
        ), 
